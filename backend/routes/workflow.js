...
async function executeStage(projectId, stageId, context = {}) {
    ...
    console.log(`[Workflow] 开始执行阶段: ${stageId} (${stage.name})`);
    ...
    console.log(`[Workflow] ✓ 阶段 ${stageId} 执行完成，生成 ${generatedArtifacts.length} 个交付物`);
...
router.post('/:projectId/execute-batch', async (req, res, next) => {
    try {
        ...
        console.log(`[Workflow] 开始批量执行 ${stageIds.length} 个阶段...`);
        ...
        console.log(`[Workflow] ✓ 批量执行完成，总消耗 ${totalTokens} tokens`);
