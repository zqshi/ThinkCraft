...
class BusinessPlanGenerator {
    constructor(apiClient, stateManager, agentProgressManager) {
        ...
        this.chapterConfig = {
            business: {
                core: [
                    { id: 'executive_summary', title: '执行摘要', desc: '业务概述、市场机会、解决方案、商业模式' },
                    { id: 'market_analysis', title: '市场分析', desc: '市场规模、目标用户、痛点分析、市场趋势' },
                    { id: 'solution', title: '解决方案', desc: '产品定位、核心功能、技术方案、差异化优势' },
                    { id: 'business_model', title: '商业模式', desc: '收入模式、定价策略、成本结构、盈利预测' }
                ],
                optional: [
                    { id: 'competitive_landscape', title: '竞争格局', desc: '竞争对手分析、差异化策略、进入壁垒' },
                    { id: 'marketing_strategy', title: '市场策略', desc: '客户获取、营销渠道、品牌定位、增长策略' },
                    { id: 'team_structure', title: '团队架构', desc: '核心团队、组织架构、人才需求、股权激励' },
                    { id: 'financial_projection', title: '财务预测', desc: '收入预测、成本预算、现金流、融资需求' },
                    { id: 'risk_assessment', title: '风险评估', desc: '市场风险、技术风险、运营风险、应对策略' },
                    { id: 'implementation_plan', title: '实施计划', desc: '里程碑规划、产品开发、市场推广、KPI设定' },
    ...
    showChapterSelection(type) {
        ...
        window.modalManager.updateContent('chapterSelectionModal', '#chapterStats',
    ...
    renderChapterList(config) {
        const coreHTML = config.core.map(ch => `
        ...
        const optionalHTML = config.optional.map(ch => `
    ...
    async generate(type, chapterIds) {
        try {
            ...
            console.log(`[BusinessPlan] 开始生成 ${type}，章节数: ${chapterIds.length}`);
            ...
            console.log(`[BusinessPlan] 生成完成，tokens: ${totalTokens}, 耗时: ${duration}s, 成本: ${costStats.costString}`);
            ...
            // 显示成功提示
            window.modalManager.alert(`生成完成！共生成 ${chapters.length} 个章节，使用 ${totalTokens} tokens，成本 ${costStats.costString}`, 'success');
            ...
            console.error('[BusinessPlan] 生成失败:', error);
    ...
    async saveReport(type, data) {
        try {
            ...
            console.log('[BusinessPlan] 报告已保存');
        } catch (error) {
            console.error('[BusinessPlan] 保存报告失败:', error);
        }
    }
    ...
    showViewReportButton(type) {
        ...
        const typeTitle = type === 'business' ? '商业计划书' : '产品立项材料';
        console.log(`[BusinessPlan] 可以查看${typeTitle}了`);
...
if (typeof window !== 'undefined') {
    ...
    console.log('[BusinessPlanGenerator] 商业计划书生成器已加载');
}
