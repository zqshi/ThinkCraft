...
class GestureHandler {
    ...
    registerSwipe(element, callbacks) {
        if (!element) {
            console.error('[GestureHandler] registerSwipe: element is required');
            return;
        }
        ...
        const handleTouchMove = (e) => {
            ...
            // 判断是否真正开始移动
            if (!hasMoved && (Math.abs(deltaX) > this.config.moveThreshold || Math.abs(deltaY) > this.config.moveThreshold)) {
                hasMoved = true;
            }
        ...
        console.log('[GestureHandler] 滑动手势已注册:', element);
    }
    ...
    registerLongPress(element, callback, duration = this.config.longPressDuration) {
        if (!element || !callback) {
            console.error('[GestureHandler] registerLongPress: element and callback are required');
            return;
        }
        ...
        console.log('[GestureHandler] 长按手势已注册:', element, 'duration:', duration + 'ms');
    }
    ...
    registerDoubleTap(element, callback) {
        if (!element || !callback) {
            console.error('[GestureHandler] registerDoubleTap: element and callback are required');
            return;
        }
        ...
        console.log('[GestureHandler] 双击手势已注册:', element);
    }
    ...
    registerPinch(element, callbacks) {
        if (!element) {
            console.error('[GestureHandler] registerPinch: element is required');
            return;
        }
        ...
        console.log('[GestureHandler] 捏合手势已注册:', element);
    }
    ...
    cleanup(element) {
        ...
        if (cleaned) {
            console.log('[GestureHandler] 已清理元素的所有手势:', element);
        }
    }
    ...
    cleanupAll() {
        ...
        console.log('[GestureHandler] 已清理所有手势，共', count, '个');
    }
    ...
    updateConfig(newConfig) {
        ...
        console.log('[GestureHandler] 配置已更新:', this.config);
    }
...
if (typeof window !== 'undefined') {
    ...
    console.log('[GestureHandler] 手势处理系统已加载');
}
