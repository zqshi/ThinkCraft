...
export async function loadSystemPrompts() {
  ...
  try {
    ...
    const apiBase =
      savedSettings.apiUrl || window.appState?.settings?.apiUrl || 'http://localhost:3000';
  ...
  try {
    const localResponse = await fetch('/prompts/scene-1-dialogue/dialogue-guide/system-default.md');
    if (!localResponse.ok) {
