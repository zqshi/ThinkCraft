...
async function generateSingleChapter(chapterId, conversationHistory) {
    ...
    console.log(`[BusinessPlan] 生成章节: ${chapterId} (${agent.name})`);
...
router.post('/generate-batch', async (req, res, next) => {
    try {
        ...
        console.log(`[BusinessPlan] 开始批量生成 ${chapterIds.length} 个章节`);
        ...
        console.log(`[BusinessPlan] ✓ 批量生成完成，耗时 ${duration}秒，tokens: ${totalTokens}`);
