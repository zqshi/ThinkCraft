...
class ShareCard {
  ...
  async getActiveReport() {
    if (window.chatReportBundle?.getActiveReport) {
      return await window.chatReportBundle.getActiveReport('analysis', {
        chatId: this.state?.currentChat,
        stateChats: this.state?.chats
      });
    }
    return window.lastGeneratedReport || null;
  }
   ...
   */
  async showShareCard() {
    const report = await this.getActiveReport();
    if (!report) {
      alert('请先生成报告');
      return;
    }
  ...
  async updateShareCard() {
    ...
    const highlights = Array.isArray(cardData.highlights)
      ...
      : Array.isArray(cardData.keyHighlights)
        ...
        : [];
    const oneLineValue =
      cardData.oneLineValue || cardData.valueProposition || cardData.valueProp || '';
    ...
    if (scoreNodes.length >= 3) {
      ...
      scoreNodes[2].textContent = Number.isFinite(scores.marketPotential)
  ...
  async generateShareLink() {
    ...
    try {
      ...
      const shareUrl =
        data?.data?.shareUrl || `${window.location.origin}/share/${data.data.shareId}`;

      // 弹窗展示分享卡片（不再仅复制）
      this.showShareCard();
      await this.prepareShareCardData();
      await this.updateShareCard();
...
function showShareCard() {
  return window.shareCard.showShareCard();
}
