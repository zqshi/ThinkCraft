...
router.post('/hire', async (req, res, next) => {
    try {
        ...
        console.log(`[Agents] 用户 ${userId} 雇佣了 ${agentConfig.name}`);
...
router.post('/assign-task', async (req, res, next) => {
    try {
        ...
        console.log(`[Agents] ${agent.nickname} 开始处理任务: ${task}`);
        ...
        console.log(`[Agents] ${agent.nickname} 完成任务，tokens: ${result.usage.total_tokens}`);
...
router.delete('/:userId/:agentId', (req, res) => {
    ...
    console.log(`[Agents] 用户 ${userId} 解雇了 ${agent.nickname}`);
