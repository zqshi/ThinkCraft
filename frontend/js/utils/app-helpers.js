 ...
 */
function showUnifiedConfirm(message, onConfirm) {
  if (window.modalManager?.openTemp) {
    const buttonId = `unified-confirm-btn-${Date.now()}`;
    const modalId = window.modalManager.openTemp({
      title: '提示',
      content: `
      ...
      footer: `
                ...
                <button class="btn btn-primary" id="${buttonId}">确定</button>
            `,
      className: 'confirm-dialog unified-confirm'
    });
...
async function requireAuth({ redirect = true, prompt = true } = {}) {
  ...
  if (prompt) {
    const message = '当前操作需要登录，是否前往登录页面？';
    showUnifiedConfirm(message, () => {
      if (redirect) {
...
window.showUnifiedConfirm = showUnifiedConfirm;
