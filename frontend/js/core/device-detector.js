...
class DeviceDetector {
    ...
    categorizeScreenSize(width) {
        ...
        if (width <= 896) return 'small-tablet';
    ...
    handleOrientationChange() {
        ...
        console.log('[DeviceDetector] 屏幕方向变化:', this.deviceType.orientation);
    ...
    handleResize() {
        ...
        if (oldDeviceType !== newDeviceType) {
            this.updateBodyClasses();
            console.log('[DeviceDetector] 设备类型变化:', oldDeviceType, '->', newDeviceType);
     ...
     */
    updateBodyClasses() {
        const { isMobile, isTablet, isDesktop, screenSize, isIOS, isAndroid, isNotchedIPhone, isIPad, isPortrait, isLandscape, browser } = this.deviceType;
        ...
        document.body.className = document.body.className
            ...
            .filter(c => !c.startsWith('device-') &&
                        ...
                        c !== 'touch-device' && c !== 'hover-device' &&
                        c !== 'ios' && c !== 'android' && c !== 'notched-iphone' &&
                        c !== 'ipad' && c !== 'portrait' && c !== 'landscape' &&
                        c !== 'has-safe-area')
            .join(' ');
        ...
        if (isNotchedIPhone) classes.push('notched-iphone');
        if (isIPad) classes.push('ipad');
        ...
        // 能力
        if (touch) classes.push('touch-device');
        if (hover) classes.push('hover-device');
        if (multiTouch) classes.push('multi-touch');
        if (safeArea) classes.push('has-safe-area');
    ...
    applyPerformanceTier() {
        ...
        if (tier === 'low') {
            ...
            console.log('[DeviceDetector] 低性能设备，已禁用动画');
    ...
    init() {
        ...
        console.log('[DeviceDetector] 设备检测完成:', {
            deviceType: this.deviceType,
        ...
        // 触发初始化完成事件
        window.dispatchEvent(new CustomEvent('deviceDetectorReady', {
            detail: {
                deviceType: this.deviceType,
                capabilities: this.capabilities,
                performance: this.performance
