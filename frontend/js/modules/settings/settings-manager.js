...
const settingsLogger =
  window.__settingsLogger ||
  (window.__settingsLogger = window.createLogger ? window.createLogger('Settings') : console);
...
class SettingsManager {
  ...
  openBottomSettings() {
    // 防止在初始化时立即触发
    if (!this.initialized) {
      settingsLogger.debug('SettingsManager not fully initialized yet');
      return;
    }
  ...
  loadSettings() {
    ...
    if (saved) {
      ...
      if (typeof state.settings.apiUrl === 'string') {
        state.settings.apiUrl = state.settings.apiUrl.replace('http://localhost:3000', 'http://127.0.0.1:3000');
      }
    }
    ...
    if (window.apiClient && window.apiClient.setBaseURL) {
      ...
      const apiUrl = state.settings.apiUrl || ((isLocalhost && window.location.port !== '3000') ? 'http://127.0.0.1:3000' : window.location.origin);
