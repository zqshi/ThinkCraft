...
class WorkflowRecommendationManager {
    constructor() {
        ...
        console.log('[WorkflowRecommendationManager] 工作流推荐管理器已初始化');
    }
    ...
    async init() {
        try {
            ...
            console.log(`[WorkflowRecommendationManager] 加载了 ${this.templates.length} 个模板`);
        } catch (error) {
            console.error('[WorkflowRecommendationManager] 初始化失败:', error);
        }
    }
    ...
    async loadTemplates() {
        try {
            ...
            console.error('[WorkflowRecommendationManager] 加载模板失败:', error);
            return [];
        }
    }
    ...
    async analyzeProject(projectName, projectDescription = '', conversation = '') {
        try {
            console.log('[WorkflowRecommendationManager] 开始分析项目...');
            ...
            console.log('[WorkflowRecommendationManager] ✓ 分析完成');

            return this.currentRecommendation;

        } catch (error) {
            console.error('[WorkflowRecommendationManager] 分析失败:', error);
            throw error;
        }
    }
    ...
    async customizeWorkflow(templateId, selectedStages) {
        try {
            ...
            console.error('[WorkflowRecommendationManager] 定制失败:', error);
            throw error;
        }
    }
    ...
    async showRecommendationDialog(projectName, ideaId, onConfirm) {
        if (!window.modalManager) {
            console.error('[WorkflowRecommendationManager] modalManager不可用');
            return;
        }
        ...
        try {
            ...
            console.error('[WorkflowRecommendationManager] 显示推荐对话框失败:', error);
            window.modalManager.close();
    ...
    renderRecommendationDialog(recommendation, onConfirm) {
        ...
        // 置信度颜色
        const confidenceColor = analysis.confidence >= 0.8 ? '#10b981' :
                                analysis.confidence >= 0.6 ? '#f59e0b' : '#ef4444';
        ...
        // 阶段列表
        const stagesHTML = recommendation.stages.map(stage => {
        ...
        const templatesHTML = this.templates.map(template => `
            <div class="template-card" style="border: 2px solid var(--border); border-radius: 12px; padding: 16px; cursor: pointer; transition: all 0.2s;"
                ...
                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                    ${template.stages.slice(0, 5).map(stageId => {
                        const stage = window.workflowExecutor?.getStageDefinition(stageId);
                        return stage ? `<span style="background: #f3f4f6; padding: 4px 10px; border-radius: 12px; font-size: 12px;">${stage.icon} ${stage.name}</span>` : '';
                    }).join('')}
    ...
    async selectTemplate(templateId, onConfirm) {
        ...
        if (!template) {
            console.error('[WorkflowRecommendationManager] 模板不存在:', templateId);
            return;
        }
    ...
    showCustomWorkflowEditor(onConfirm) {
        ...
        // 所有可用的阶段
        const allStages = [
            { id: 'requirement', name: '需求分析', icon: '📋', description: '产品定位、用户分析、功能规划' },
            { id: 'design', name: '产品设计', icon: '🎨', description: 'UI/UX设计、交互原型、视觉规范' },
            { id: 'architecture', name: '架构设计', icon: '🏗️', description: '系统架构、技术选型、API规范' },
            { id: 'development', name: '开发实现', icon: '💻', description: '前后端开发、功能实现、代码编写' },
            { id: 'testing', name: '测试验证', icon: '🧪', description: '功能测试、性能测试、bug修复' },
            { id: 'deployment', name: '部署上线', icon: '🚀', description: '环境配置、服务器部署、上线发布' },
...
if (typeof window !== 'undefined') {
    ...
    console.log('[WorkflowRecommendationManager] 工作流推荐管理器已加载');
}
